...
get_crashTotal_current
    ...
    #查询当前崩溃总数
    ${init_issueCrashReporter_list}    get_CrashTotal_current    ${app_id}
...
send_android_java_crash
    ...
    editJson    ${android_java_crash.json}    device_id=${device_id}    crash_stack=${crash_stack}
    : FOR    ${I}    IN RANGE    1
...
get_crashTotal_after_send_sdkEvent_and_assert
    [Tags]    sdk2influxDB
    #调用自定义关键字，查询目前崩溃总数
    ${after_issueCrashReporter_list}    get_CrashTotal_current    ${app_id}
...
get_issues_list_current
    [Tags]    query
    #查询某页的issue列表
    get_issues_list_current    ${app_id}    1

get_issueTimeRange_current
    [Tags]    query
    #查询某issue的最早和最近时间
    get_issueTimeRange_current    ${app_id}

get_crashReporterPerDay_current
    [Tags]    query
    #查询每日崩溃数和影响人数
    get_crashReporterPerDay_current    ${app_id}

send_ios_crash
    ${user_trace}    getUserTrace    ios
    editJson    ${ios_crash.json}    free_ram=131072    user_trace=${user_trace}
    sleep    5
    postJson    ${ios_crash.json}    ${sdk_ip}
